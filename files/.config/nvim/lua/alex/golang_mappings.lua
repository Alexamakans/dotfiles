...
local function addJsonTags(field_declaration_list_node)
	...
	for field_declaration, _ in node:iter_children() do
		if field_declaration:named_child_count() == 2 then
			...
			local jsonTag = '`json:"' .. nameFirstLetterLowercase .. '"`'
			vim.api.nvim_buf_set_text(0, end_row, end_col, end_row, end_col, { jsonTag })
		end
	end
end
...
local function get_field_declaration_list_node(args)
	...
	if node:type() == "field_identifier" or node:type() == "type_identifier" then
		...
		if parent == nil then
			echo("failed because parent() of '" .. node:type() .. "' wasn't of type 'field_declaration', is nil")
			return
	...
	if node:type() == "field_declaration" then
		...
		if node == nil or node:type() ~= "field_declaration_list" then
			local err = "failed because parent() of 'field_declaration' wasn't of type 'field_declaration_list'"
			if node == nil then
				err = err .. ", is nil"
			else
